...
export class SoaRepository extends BaseRepository {
  private readonly memory = new Map<string, SoaDto>();

  async create(firmId: string, dto: CreateSoaDto): Promise<SoaDto> {
    const now = new Date().toISOString();
    const row: SoaDto = { id: randomUUID(), firmId, name: dto.name, metadata: dto.metadata, createdAt: now, updatedAt: now };
  ...
  async findById(firmId: string, id: string): Promise<SoaDto> {
    ...
    if (!r || r.firmId !== firmId) throw new NotFoundError('Soa', id);
    return r;
  }

  async list(firmId: string, opts: { cursor?: string; limit: number }): Promise<{ items: SoaDto[]; nextCursor: string | null }> {
  ...
  async update(firmId: string, id: string, dto: UpdateSoaDto): Promise<SoaDto> {
    ...
    const updated: SoaDto = { ...cur, ...dto, updatedAt: new Date().toISOString() };
