...
import type { TenancyDto, CreateTenancyDto, UpdateTenancyDto } from './dto.js';

@Injectable()
export class TenancyRepository extends BaseRepository {
  private readonly memory = new Map<string, TenancyDto>();
  ...
  async findById(firmId: string, id: string): Promise<TenancyDto> {
    const r = this.memory.get(id);
    if (!r || r.firmId !== firmId) throw new NotFoundError('Tenancy', id);
    return r;
  }

  async list(firmId: string, opts: { cursor?: string; limit: number }): Promise<{ items: TenancyDto[]; nextCursor: string | null }> {
