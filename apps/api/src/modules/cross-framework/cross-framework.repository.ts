...
export class CrossFrameworkRepository extends BaseRepository {
  private readonly memory = new Map<string, CrossFrameworkDto>();

  async create(firmId: string, dto: CreateCrossFrameworkDto): Promise<CrossFrameworkDto> {
    const now = new Date().toISOString();
    const row: CrossFrameworkDto = { id: randomUUID(), firmId, name: dto.name, metadata: dto.metadata, createdAt: now, updatedAt: now };
  ...
  async findById(firmId: string, id: string): Promise<CrossFrameworkDto> {
    ...
    if (!r || r.firmId !== firmId) throw new NotFoundError('CrossFramework', id);
    return r;
  }

  async list(firmId: string, opts: { cursor?: string; limit: number }): Promise<{ items: CrossFrameworkDto[]; nextCursor: string | null }> {
  ...
  async update(firmId: string, id: string, dto: UpdateCrossFrameworkDto): Promise<CrossFrameworkDto> {
    ...
    const updated: CrossFrameworkDto = { ...cur, ...dto, updatedAt: new Date().toISOString() };
