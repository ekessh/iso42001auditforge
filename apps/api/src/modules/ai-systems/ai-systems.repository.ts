...
export class AiSystemsRepository extends BaseRepository {
  private readonly memory = new Map<string, AiSystemsDto>();

  async create(firmId: string, dto: CreateAiSystemsDto): Promise<AiSystemsDto> {
    const now = new Date().toISOString();
    const row: AiSystemsDto = { id: randomUUID(), firmId, name: dto.name, metadata: dto.metadata, createdAt: now, updatedAt: now };
  ...
  async findById(firmId: string, id: string): Promise<AiSystemsDto> {
    ...
    if (!r || r.firmId !== firmId) throw new NotFoundError('AiSystems', id);
    return r;
  }

  async list(firmId: string, opts: { cursor?: string; limit: number }): Promise<{ items: AiSystemsDto[]; nextCursor: string | null }> {
  ...
  async update(firmId: string, id: string, dto: UpdateAiSystemsDto): Promise<AiSystemsDto> {
    ...
    const updated: AiSystemsDto = { ...cur, ...dto, updatedAt: new Date().toISOString() };
