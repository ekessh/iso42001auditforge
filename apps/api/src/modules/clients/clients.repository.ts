...
export class ClientsRepository extends BaseRepository {
  ...
  async create(firmId: string, dto: CreateClientsDto): Promise<ClientsDto> {
    ...
    const row: ClientsDto = { id: randomUUID(), firmId, name: dto.name, metadata: dto.metadata, createdAt: now, updatedAt: now };
  ...
  async list(firmId: string, opts: { cursor?: string; limit: number }): Promise<{ items: ClientsDto[]; nextCursor: string | null }> {
  ...
  async update(firmId: string, id: string, dto: UpdateClientsDto): Promise<ClientsDto> {
    ...
    const updated: ClientsDto = { ...cur, ...dto, updatedAt: new Date().toISOString() };
