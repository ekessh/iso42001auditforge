...
export type CandidateFindingId = Brand<Uuid, 'CandidateFindingId'>;
export type CandidateFindingDecisionId = Brand<
  Uuid,
  'CandidateFindingDecisionId'
>;
export type CandidateFindingEvidenceLinkId = Brand<
  Uuid,
  'CandidateFindingEvidenceLinkId'
>;
...
export function asControlId(s: string): ControlId {
  if (!s || s.length === 0) throw new TypeError('ControlId: empty');
  return s as ControlId;
}
...
export type { EngagementId, FirmId, FindingId };
